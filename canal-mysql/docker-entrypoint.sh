...
docker_setup_env() {
	...
	file_env 'CANAL_ADMIN_DATABASE'
	file_env 'CANAL_ADMIN_USER'
	file_env 'CANAL_ADMIN_PASSWORD'
...
docker_process_sql() {
	...
	if [ -n "$CANAL_ADMIN_DATABASE" ]; then
		set -- --database="$CANAL_ADMIN_DATABASE" "$@"
	fi
...
docker_setup_db() {
	...
	if [ -n "$CANAL_ADMIN_DATABASE" ]; then
		mysql_note "Creating database ${CANAL_ADMIN_DATABASE}"
		docker_process_sql --database=mysql <<<"CREATE DATABASE IF NOT EXISTS \`$CANAL_ADMIN_DATABASE\` ;"
	fi

	if [ -n "$CANAL_ADMIN_USER" ] && [ -n "$CANAL_ADMIN_PASSWORD" ]; then
		mysql_note "Creating user ${CANAL_ADMIN_USER}"
		docker_process_sql --database=mysql <<<"CREATE USER '$CANAL_ADMIN_USER'@'%' IDENTIFIED BY '$CANAL_ADMIN_PASSWORD' ;"

		if [ -n "$CANAL_ADMIN_DATABASE" ]; then
			mysql_note "Giving user ${CANAL_ADMIN_USER} access to schema ${CANAL_ADMIN_DATABASE}"
			docker_process_sql --database=mysql <<<"GRANT ALL ON \`${CANAL_ADMIN_DATABASE//_/\\_}\`.* TO '$CANAL_ADMIN_USER'@'%' ;"
		fi
